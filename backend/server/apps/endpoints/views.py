# For each model, we created a view which will allow to retrieve single object or
# list of objects. We will not allow to create or modify Endpoints, MLAlgorithms
# by REST API. The code to to handle creation of new ML related objects will be on
# server side, I will describe it in the next chapter.
#
# We will allow to create MLAlgorithmStatus objects by REST API. We don't allow to
# edit statuses for ML algorithms as we want to keep all status history.
#
# We allow to edit MLRequest objects, however only feedback field (please take a
# look at serializer definition)
...
# AB Test
# The ABTestViewSet view allows the user to create new objects. The perform_create
# method creates the ABTest object and two new statuses for ML algorithms. The new
# statuses are set to ab_testing.
...
# The StopABTestView stops the A/B test and compute the accuracy (ratio of correct 
# responses) for each algorithm. The algorithm with higher accurcy is set as production
# algorithm, the other algorithm is saved with testing status.
